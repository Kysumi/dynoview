import { ListTablesCommand, DynamoDBDocumentClient, DynamoDBClient } from '@aws-sdk/client-dynamodb'
...
export const listAvailableTables = async ({
  ...
  try {
    ...
    if (
      e instanceof Error &&
      (e.message.includes('credentials') || e.message.includes('authentication'))
    ) {
